...
app.get('/buyLinkCallback', (req, res) => {
  let response = util.format(
    'Received payment callback with RefNo: %s',
    req.query.refno
  );
...
app.post('/subscription', (req, ret) => {
  ...
  if (typeof request.RefNo !== 'undefined') {
    tco
      .subscription()
      .getSubscriptionsByOrderRefNo(request.RefNo, (err, res) => {
        if (res !== null) {
          let firstSubscriptionRefId =
            res[Object.keys(res)[0]][0]['SubscriptionReference'];
          ret.json({ payload: res, subscriptionRefId: firstSubscriptionRefId });
        }
        if (err !== null) {
          ret.json({
            error:
              "Couldn't find subscription reference ID for RefNo: " +
              request.RefNo,
          });
        }
      });
...
app.post('/disable_subscription', (req, ret) => {
  ...
  if (typeof request.subscriptionRefId !== 'undefined') {
    tco
      .subscription()
      .disable({ subRefId: request.subscriptionRefId }, (err, res) => {
        if (res !== null) {
          ret.json({
            payload: res,
            subscriptionRefId: request.subscriptionRefId,
          });
        }
        if (err !== null) {
...
app.post('/enable_subscription', (req, ret) => {
  ...
  if (typeof request.subscriptionRefId !== 'undefined') {
    tco
      .subscription()
      .enable({ subRefId: request.subscriptionRefId }, (err, res) => {
        if (res !== null) {
          ret.json({
            payload: res,
            subscriptionRefId: request.subscriptionRefId,
...
app.post('/buyLink', (req, ret) => {
  ...
  if (typeof request.buy_link !== 'undefined' && request.buy_link === 'true') {
    let finalParams = JSON.parse(
      JSON.stringify(simple_buy_link_payment_params)
    );
