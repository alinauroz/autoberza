    ...
    if (items.length > 0) {
      Object.keys(items).forEach((key) => {
        ...
        if (
          ...
          const productSubcriptions =
            product['ProductDetails']['Subscriptions'];
  ...
  async getSubscriptionsByOrderRefNo(orderRefNo, callback) {
    ...
    if (typeof callback !== undefined && typeof callback === 'function') {
      if (typeof orderRefNo === 'string' && orderRefNo.length <= 10) {
        ...
        callback(
          new TwocheckoutError(
            'Order reference No is not a string or the size is to big'
          ),
          null
        );
      }
      ...
      return new Promise((resolve, reject) => {
        if (typeof orderRefNo === 'string' && orderRefNo.length <= 10) {
          order
            ...
            .then((orderData) => {
              const subscriptions = this.getSubscriptionFromItems(
                orderData['Items']
              );
          ...
          reject(
            new TwocheckoutError(
              'Order reference No is not a string or the size is too big'
            )
          );
        }
      });
