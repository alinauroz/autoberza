...
class IpnSignature {
  ...
  isValid(params) {
    try {
      ...
      Object.keys(params).forEach((key) => {
        let val = params[key];
        if (
          !['HASH', 'SIGNATURE_SHA2_256', 'SIGNATURE_SHA3_256'].includes(key)
        ) {
  ...
  calculateIpnResponse(ipnParams) {
    try {
      ...
      ipnParamsResponse.DATE = dateFormat.asString(
        'yyyyMMddhhmmss',
        new Date()
      );
  ...
  hmac(string, algo) {
    const hash = crypto
      .createHmac(algo, this.config.secretKey)
      .update(string)
      .digest('hex');
    return hash;
  }
  ...
  formatResponse(algorithm, date, signature) {
    if (algorithm == 'md5') {
      return util.format('<EPAYMENT>%s|%s</EPAYMENT>', date, signature);
    } else {
      return util.format(
